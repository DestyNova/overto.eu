...
+++<del>+++At the moment I'd say Nim is my favourite general-purpose language to work with, presenting an excellent balance between conciseness, expressivity, reliability, performance and usability.+++</del>+++ 

__(Note: As of mid-2026 I stopped using Nim because the project has allowed LLM-generated code into the compiler and standard library https://github.com/nim-lang/Nim/pull/25832#issuecomment-4507847576[without a second thought]. I can't trust code produced by LLMs or projects that rely on it.)__

=== Nim programs

* https://github.com/DestyNova/nimplication[Nimplication], a toy SAT solver using DPLL.
...
* https://github.com/DestyNova/memvalid[memvalid], a stateless little utility for memorising text from scripts, songs etc. It was hosted on Heroku until they pulled their free tier, now it's https://memvalid.overto.eu[running on a tiny VPS] using the excellent https://github.com/swiftwave-org/swiftwave[Swiftwave].
